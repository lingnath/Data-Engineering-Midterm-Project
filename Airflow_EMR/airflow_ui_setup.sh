...
access_key=$(cat .env | grep 'ACCESS_KEY' | awk -F " = " '{print $2}' | tr -d '"')
secret_access_key=$(cat .env | grep 'SECRET_KEY' | awk -F " = " '{print $2}' | tr -d '"')
region=$(cat config_file.toml | grep 'region' | awk -F"=" '{print $2}' | tr -d "'")
dag_name=$(cat config_file.toml | grep 'dag_name' | awk -F"=" '{print $2}' | tr -d "'")
