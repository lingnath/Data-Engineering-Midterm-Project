...
athena_db=$(cat ../config_file.toml | grep 'athena_db' | awk -F"=" '{print $2}' | tr -d "'")
s3_bucket_output=$(cat ../config_file.toml | grep 's3_bucket_output' | awk -F"=" '{print $2}' | tr -d "'")
aws_region=$(cat ../config_file.toml | grep 'region' | awk -F"=" '{print $2}' | tr -d "'")
glue_crawler_name=$(cat ../config_file.toml | grep 'glue_crawler_name' | awk -F"=" '{print $2}' | tr -d "'")
